...
class ServiceManager:
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'surl', self.services['surl']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8080 -e TZ=Asia/Seoul -v /dockerProjects/surl/volumes/gen:/gen --pull always ghcr.io/qBlackBirdp/surl")
